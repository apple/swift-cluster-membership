...
public final class SWIMNIOShell: Sendable {
  ...
  private let pendingReplyCallbacks:
    Mutex<
  ...
  public func receiveMessage(message: SWIM.Message, from address: SocketAddress) {
    ...
    case .response(let response):
        ...
        let callbackKey = PendingResponseCallbackIdentifier(
          peerAddress: address, sequenceNumber: response.sequenceNumber, inResponseTo: nil)
      #else
        let callbackKey = PendingResponseCallbackIdentifier(
          peerAddress: address, sequenceNumber: response.sequenceNumber)
      #endif
        ...
        self.log.trace(
          ...
          metadata: [
            "pending/callbacks": Logger.MetadataValue.array(
              self.pendingReplyCallbacks.withLock { $0.map { "\($0)" } })
          ])
        self.swim.metrics.shell.pingResponseTime.recordNanoseconds(
  ...
  internal func registerCallback(for writeCommand: SWIMNIOWriteCommand) {
    ...
    case .wait(let reply, _):
      ...
      let timeoutTask = self.eventLoop.scheduleTask(in: reply.timeout) {
        if let callback = self.pendingReplyCallbacks.withLock({
          $0.removeValue(forKey: callbackKey)
        }) {
          callback(
            .failure(
              SWIMNIOTimeoutError(
                ...
                message:
                  "Timeout of [\(callbackKey)], no reply to [\(writeCommand.message.messageCaseDescription)] after \(reply.timeout.prettyDescription())"
              )
            ))
