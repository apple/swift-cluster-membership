...
extension TestMetrics {
    ...
    -> TestCounter
  {
    ...
    if let c: CounterHandler = self.counters.withLock({
      $0[.init(label: label, dimensions: dimensions)]
    }) {
      counter = c
    } else {
  ...
  public func expectRecorder(_ label: String, _ dimensions: [(String, String)] = []) throws
    -> TestRecorder
  {
    guard let counter = self.recorders.withLock({ $0[.init(label: label, dimensions: dimensions)] })
    else {
      throw TestMetricsError.missingMetric(label: label, dimensions: [])
    }
  ...
  public func expectTimer(_ label: String, _ dimensions: [(String, String)] = []) throws
    -> TestTimer
  {
    guard let counter = self.timers.withLock({ $0[.init(label: label, dimensions: dimensions)] })
    else {
      throw TestMetricsError.missingMetric(label: label, dimensions: [])
    }
