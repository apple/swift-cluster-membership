...
extension SWIM.RemoteMessage: InternalProtobufRepresentable {
    ...
    init(fromProto proto: ProtobufRepresentation, context: Serialization.Context) throws {
        ...
        case .ping(let ping):
            let replyTo = try Peer<SWIM.PingResponse>(fromProto: ping.replyTo, context: context)
        ...
        case .pingRequest(let pingRequest):
            let target = try Peer<SWIM.Message>(fromProto: pingRequest.target, context: context)
            let replyTo = try Peer<SWIM.PingResponse>(fromProto: pingRequest.replyTo, context: context)
...
extension SWIM.Member: InternalProtobufRepresentable {
    ...
    init(fromProto proto: ProtoSWIMMember, context: Serialization.Context) throws {
        let address = try PeerAddress(fromProto: proto.address, context: context)
        let ref = context.resolvePeer(SWIM.Message.self, identifiedBy: address)
...
extension SWIM.PingResponse: InternalProtobufRepresentable {
    ...
    init(fromProto proto: ProtoSWIMPingResponse, context: Serialization.Context) throws {
        ...
        switch pingResponse {
        case .ack(let ack):
            let target = context.resolvePeer(SWIM.Message.self, identifiedBy: try PeerAddress(fromProto: ack.target, context: context))
        ...
        case .nack(let nack):
            let target = context.resolvePeer(SWIM.Message.self, identifiedBy: try PeerAddress(fromProto: nack.target, context: context))
